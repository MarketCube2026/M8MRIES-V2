...
金额单位为万元、精度四位小数。旧评分保留原分，未知满分不换算百分制。V1 历史记录默认标记为已审核，原始状态始终保留。
...
按业务确认规则，V1 未提供独立审批金额时使用 support_amount 作为默认审批金额，并生成审批记录和投入台账；实际投入仍保持缺失。审批时间优先使用 approved_at，其次使用 updated_at、created_at 或会议日期。审批满一个月且尚无复盘记录的项目进入会后复盘待办。
