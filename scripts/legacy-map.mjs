...
export function mapLegacy(row, {batch,ownerMap={},source='V1'} = {}) {
  ...
  const approved=amount(row.approved_amount??raw.approvedAmount);
  ...
  const record={id,projectId:'PRJ-'+id,applicationNo:'LEGACY-'+id,
    ...
    originalSnapshot:row,status:states[status]??'REVIEW_REQUIRED',
  ...
  if(approved!==null || actual!==null)record.ledgerEntries={create:{
    requestedAmount:requested,recommendedAmount:recommended,approvedAmount:approved,actualAmount:actual,
    note:text(row.note??raw.note??raw.budgetNote??row.budget_note),year:date?new Date(date).getFullYear():new Date().getFullYear(),
    region:record.region,hospital:record.hospital,kol:record.kol}};
